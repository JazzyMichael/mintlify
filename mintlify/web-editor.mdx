...
Missed a great opportunity to add a simple animation to the expand and collapse states of the sidebar in the dashboard.
...
- transition: width 150ms ease-in, margin-left 150ms ease-in;
  notes:
- flexbox simplifies this. Instead of having manual values of margin equal to the sidenav width, we can just set the content container to be full width so it automatically adjusts as the sidenav width changes. This removes the overhead of managing margin, and removes the need for a separate transition to be applied to the content container (the transition in the sidenav causes all children in the flexbox scope to adjust automatically at the same transition/animation speed)
